...
# ============================================================
# API REST – ASEGURADORAS (CRUD completo)
# ============================================================
...
@app.route("/api/aseguradoras/<int:id>", methods=["GET"])
def api_obtener_aseguradora(id):
    db = get_db()
    try:
        with db.cursor() as cur:
            cur.execute("SELECT * FROM aseguradora WHERE id=%s", (id,))
            row = cur.fetchone()
        if not row:
...
def api_crear_aseguradora():
    ...
    try:
        with db.cursor() as cur:
            cur.execute("""
                INSERT INTO aseguradora (nombre, telefono, email, direccion)
                VALUES (%s, %s, %s, %s)
            """, (data["nombre"], data.get("telefono"),
...
@app.route("/api/aseguradoras/<int:id>", methods=["PUT"])
def api_actualizar_aseguradora(id):
    data = request.get_json(silent=True) or {}
    db = get_db()
    try:
        with db.cursor() as cur:
        ...
        db.commit()
        return ok(None, 200, "Aseguradora actualizada")
    except Exception as e:
        db.rollback()
        return error(str(e))
    finally:
...
@app.route("/api/aseguradoras/<int:id>", methods=["DELETE"])
def api_eliminar_aseguradora(id):
    db = get_db()
    try:
        with db.cursor() as cur:
            cur.execute("UPDATE aseguradora SET activa=0 WHERE id=%s", (id,))
        db.commit()
        return ok(None, 200, "Aseguradora desactivada")
    except Exception as e:
        db.rollback()
        return error(str(e))
    finally:
...
# ============================================================
# API REST – TIPO CASO (CRUD completo)
# ============================================================

@app.route("/api/tipo_caso", methods=["GET"])
def api_tipo_caso():
    db = get_db()
    try:
        with db.cursor() as cur:
            cur.execute("SELECT * FROM tipo_caso WHERE activo=1 ORDER BY nombre")
            return ok(filas(cur.fetchall()))
    except Exception as e:
        return error(str(e))
    finally:
...
@app.route("/api/tipo_caso/<int:id>", methods=["GET"])
def api_obtener_tipo_caso(id):
    db = get_db()
    try:
        with db.cursor() as cur:
            cur.execute("SELECT * FROM tipo_caso WHERE id=%s", (id,))
            row = cur.fetchone()
        if not row:
            return error("Tipo de caso no encontrado", 404)
        return ok(filas([row])[0])
    except Exception as e:
        return error(str(e))
    finally:
...
@app.route("/api/tipo_caso", methods=["POST"])
def api_crear_tipo_caso():
    data = request.get_json(silent=True) or {}
    if not data.get("nombre"):
        return error("Campo obligatorio: nombre")
    db = get_db()
    try:
        with db.cursor() as cur:
            cur.execute("""
                INSERT INTO tipo_caso (nombre, descripcion)
                VALUES (%s, %s)
            """, (data["nombre"], data.get("descripcion")))
        db.commit()
        return ok({"id": cur.lastrowid}, 201, "Tipo de caso creado")
    except Exception as e:
        db.rollback()
        return error(str(e))
    finally:
...
@app.route("/api/tipo_caso/<int:id>", methods=["PUT"])
def api_actualizar_tipo_caso(id):
    data = request.get_json(silent=True) or {}
    db = get_db()
    try:
        with db.cursor() as cur:
        ...
        db.commit()
        return ok(None, 200, "Tipo de caso actualizado")
    except Exception as e:
        db.rollback()
        return error(str(e))
    finally:
...
@app.route("/api/tipo_caso/<int:id>", methods=["DELETE"])
def api_eliminar_tipo_caso(id):
    db = get_db()
    try:
        with db.cursor() as cur:
            cur.execute("UPDATE tipo_caso SET activo=0 WHERE id=%s", (id,))
        db.commit()
        return ok(None, 200, "Tipo de caso desactivado")
    except Exception as e:
        db.rollback()
        return error(str(e))
    finally:
...
# ============================================================
# API REST – LICENCIADO (CRUD completo)
# ============================================================

@app.route("/api/licenciados", methods=["GET"])
def api_licenciados():
    db = get_db()
    try:
        with db.cursor() as cur:
            cur.execute("SELECT * FROM licenciado WHERE activo=1 ORDER BY nombre_completo")
            return ok(filas(cur.fetchall()))
    except Exception as e:
        return error(str(e))
    finally:
...
@app.route("/api/licenciados/<int:id>", methods=["GET"])
def api_obtener_licenciado(id):
    db = get_db()
    try:
        with db.cursor() as cur:
            cur.execute("SELECT * FROM licenciado WHERE id=%s", (id,))
            row = cur.fetchone()
        if not row:
            return error("Licenciado no encontrado", 404)
        return ok(filas([row])[0])
    except Exception as e:
        return error(str(e))
    finally:
...
@app.route("/api/licenciados", methods=["POST"])
def api_crear_licenciado():
    data = request.get_json(silent=True) or {}
    if not data.get("nombre_completo") or not data.get("cedula"):
        return error("Campos obligatorios: nombre_completo, cedula")
    db = get_db()
    try:
        with db.cursor() as cur:
            cur.execute("""
                INSERT INTO licenciado (nombre_completo, cedula, telefono, email, especialidad)
                VALUES (%s, %s, %s, %s, %s)
            """, (data["nombre_completo"], data["cedula"], data.get("telefono"),
                  data.get("email"), data.get("especialidad")))
        db.commit()
        return ok({"id": cur.lastrowid}, 201, "Licenciado creado")
    except Exception as e:
        db.rollback()
        return error(str(e))
    finally:
...
@app.route("/api/licenciados/<int:id>", methods=["PUT"])
def api_actualizar_licenciado(id):
    data = request.get_json(silent=True) or {}
    db = get_db()
    try:
        with db.cursor() as cur:
            cur.execute("""
                UPDATE licenciado
                SET nombre_completo=%s, cedula=%s, telefono=%s, email=%s, especialidad=%s
                WHERE id=%s
            """, (data.get("nombre_completo"), data.get("cedula"), data.get("telefono"),
                  data.get("email"), data.get("especialidad"), id))
        db.commit()
        return ok(None, 200, "Licenciado actualizado")
    except Exception as e:
        db.rollback()
        return error(str(e))
    finally:
...
@app.route("/api/licenciados/<int:id>", methods=["DELETE"])
def api_eliminar_licenciado(id):
    db = get_db()
    try:
        with db.cursor() as cur:
            cur.execute("UPDATE licenciado SET activo=0 WHERE id=%s", (id,))
        db.commit()
        return ok(None, 200, "Licenciado desactivado")
    except Exception as e:
        db.rollback()
        return error(str(e))
    finally:
